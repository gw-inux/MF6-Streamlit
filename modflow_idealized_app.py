...
# Reuse the MODPATH 7 locator from the existing MF6-Streamlit repository.
from mf6_model import locate_mp7
...
def native_run_semaphore() -> threading.BoundedSemaphore:
    """Limit simultaneous native solver processes on Community Cloud."""
...
def _show_matplotlib(fig) -> None:
    """Render and close a figure under a process-wide Matplotlib lock."""
...
def locate_mfnwt() -> Path:
    """Locate MODFLOW-NWT for Streamlit Cloud and local development.

    Cloud deployment expects the Linux binary at ``bin/mfnwt``.  Local
    development may instead provide ``MFNWT_EXECUTABLE`` or put MODFLOW-NWT on
    PATH.  No user-facing executable path is required in the app.
    """
    candidates = []
...
def cleanup_workspace(path) -> None:
    """Remove a previous session workspace without affecting other sessions."""
    if not path:
        return
    try:
        shutil.rmtree(Path(path), ignore_errors=True)
    except Exception:
        # Cleanup must never hide a valid model result or break the UI.
        pass
...
# Helper functions
# ------------------------------------------------------------
def make_package_registry(active_wel=False, active_riv=False):
    """
...
def make_boundary_features(nrow, ncol, delr, delc, active_wel=False, well_row=None, well_col=None, active_riv=False, riv_cells=None):
    """
...
def run_modpath_from_existing_model(
    ...
    if not partlocs:
        raise ValueError("Select at least one particle starting location before running MODPATH.")
...
with col3:
    with st.expander("Model settings", expanded=True):
        st.write("**Numerical engines**")
        try:
            mfnwt_status_path = locate_mfnwt()
            st.success("MODFLOW-NWT available")
            st.caption(f"`{mfnwt_status_path.name}`")
        except Exception as exc:
            st.error("MODFLOW-NWT not available")
            st.caption(str(exc).split("\n")[0])

        try:
            mp7_status_path = locate_mp7()
            st.success("MODPATH 7 available")
            st.caption(f"`{Path(mp7_status_path).name}`")
        except Exception as exc:
            st.warning("MODPATH 7 not available")
            st.caption(str(exc).split("\n")[0])

        st.caption(
            "Model runs use isolated temporary workspaces, so simultaneous "
            "Streamlit sessions do not overwrite one another."
        )

recharge = recharge_mm_a / 1000.0 / 365.25  # mm/a -> m/d

package_registry = make_package_registry(active_wel=active_wel, active_riv=active_riv)
...
# ------------------------------------------------------------
# Workspace and stale-result handling
# ------------------------------------------------------------
modelname = "rect_chd_rch_nwt"

# Each successful flow-model run gets its own temporary workspace.  It remains
# available for MODPATH during the session and is removed when the flow-model
# inputs change or a replacement MODFLOW run is started.
if (
...
if st.button("Run MODFLOW-NWT model"):
    ...
    ws = Path(tempfile.mkdtemp(prefix="rect_chd_rch_nwt_"))
    ...
    try:
        ...
        # UPW: unconfined aquifer.  IPHDRY=1 is required when MODPATH is used
        # with MODFLOW-NWT so dry cells are written as HDRY in the head file.
        ...
        # Solver -- unchanged from the local educational model.
        ...
        # Output control. Compact binary cell-by-cell output is retained for
        ...
        mf.write_input()
        with st.spinner("Running MODFLOW-NWT..."):
            with native_run_semaphore():
        ...
        st.session_state.model_info = {
            ...
            "riv_cells": riv_cells,
            "modelname": modelname,
            "mfnwt_executable": str(mfnwt_exe),
            "modflow_console": "\n".join(buff),
        }
        ...
        st.session_state.pathlines = None
        st.session_state.particle_source_lookup = None
        st.error("MODFLOW-NWT model run failed.")
        st.exception(exc)
...
if st.session_state.model_done:
    ...
    if active_modpath:
        ...
        with col_mp1:
            try:
                mp7_ui_path = Path(locate_mp7())
                st.success("MODPATH 7 ready")
                st.caption(f"`{mp7_ui_path.name}`")
            except Exception:
                st.error("MODPATH 7 unavailable")
...
with st.expander("Deployment diagnostic"):
    try:
        mfnwt_path = locate_mfnwt()
        st.write(f"MODFLOW-NWT executable: `{mfnwt_path}`")
        st.success("MODFLOW-NWT is available to the Streamlit process.")
    except Exception as exc:
        st.error("MODFLOW-NWT executable is not available.")
        st.exception(exc)

    try:
        mp7_path = Path(locate_mp7())
        st.write(f"MODPATH 7 executable: `{mp7_path}`")
        st.success("MODPATH 7 is available to the Streamlit process.")
    except Exception as exc:
        st.error("MODPATH 7 executable is not available.")
        st.exception(exc)

    if st.session_state.model_done and st.session_state.model_info.get("modflow_console"):
        with st.expander("Last MODFLOW-NWT console output"):
            st.code(st.session_state.model_info["modflow_console"], language="text")
